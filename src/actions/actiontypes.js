//write the action types here for later exporting to prevent odd errors

export const MAKE_REQUEST = "MAKE REQUEST";
export const GET_USER_INFO = "GET USER INFO";
export const SIGN_UP = "SIGN UP";
export const SIGN_IN = "SIGN_IN";
export const ADD_ALL_ITEMS = "ADD ALL ITEMS";
export const GET_USER_ITEMS = "GET USER ITEMS";
export const ADD_ITEM_TO_EDIT = "ADD ITEM TO EDIT";
export const CLEAR_CHOSEN_ITEM = "CLEAR CHOSEN ITEM"
// export const GET_USER_ITEMS_SUCCESS  = 
export const RE_RENDER_USER_ITEMS = "RE RENDER USER ITEMS";
export const TURN_ON_EDIT = "TURN ON EDIT";
export const TURN_OFF_EDIT = "TURN OFF EDIT";
export const EDIT_ITEM = "EDIT ITEM";
export const DELETE_ITEM = "DELETE ITEM";
export const BID_ITEM = "BID ITEM";
export const BUYOUT_ITEM = "BUYOUT ITEM";